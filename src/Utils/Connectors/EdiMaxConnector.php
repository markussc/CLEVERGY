...
class EdiMaxConnector
{
        ...
        if (array_key_exists('edimax', $this->connectors) && is_array($this->connectors['edimax'])) {
            foreach ($this->connectors['edimax'] as $device) {
                ...
                $results[] = [
                    'ip' => $device['ip'],
                    'name' => $device['name'],
                    'status' => $this->createStatus($this->em->getRepository('App:EdiMaxDataStore')->getLatest($device['ip'])),
                    'nominalPower' => $nominalPower,
                    'autoIntervals' => $autoIntervals,
                    'mode' => $mode,
                    'activeMinutes' => $this->em->getRepository('App:EdiMaxDataStore')->getActiveDuration($device['ip'], $today, $now),
                ];
            }
        }
    ...
    public function switchOK($deviceId)
    {
        ...
        if ($this->em->getRepository('App:Settings')->getMode($this->connectors['edimax'][$deviceId]['ip']) == Settings::MODE_MANUAL) {
            return false;
        }
        ...
        $oldStatus = $this->em->getRepository('App:EdiMaxDataStore')->getLatest($this->connectors['edimax'][$deviceId]['ip'], ($currentStatus + 1)%2);
