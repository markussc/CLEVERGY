...
class SmartFoxDataStoreRepository extends DataStoreBaseRepository
{
    ...
    public function getEnergyInterval($ip, $parameter, $start = null, $end = null)
    {
        ...
        if ($parameter === 'PvEnergy') {
            if (is_array($endEnergy[0]->getData()[$parameter])) {
                $end = array_sum($endEnergy[0]->getData()[$parameter]);
            } else {
                $end = $endEnergy[0]->getData()[$parameter];
            }
            if (is_array($startEnergy[0]->getData()[$parameter])) {
                $start = array_sum($startEnergy[0]->getData()[$parameter]);
            } else {
                $start = $startEnergy[0]->getData()[$parameter];
            }
            return $end - $start;
    ...
    {
        $date = max(new \DateTime('-6 month'), new \DateTime('2024-06-23')); // we take 3 months as input for our training

        $em = $this->getEntityManager();
        $connection = $em->getConnection();
        $sql = "SELECT `json_value` FROM `data_store` WHERE `timestamp` >= '" . $date->format("Y-m-d") . "' AND `discr_type` LIKE 'smartfoxdatastore' AND MINUTE(timestamp) % 30 = 0 AND HOUR(timestamp) > 5 AND HOUR(timestamp) < 21";
